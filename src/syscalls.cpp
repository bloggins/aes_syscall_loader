...
static std::vector<uint8_t> g_ntdllFile;   
static std::vector<void*>    g_stubPages;  
...
static NtFn BuildStub(uint16_t ssn)
{
    ...
    void* mem = VirtualAlloc(nullptr, sizeof(bytes), MEM_COMMIT | MEM_RESERVE, PAGE_EXECUTE_READWRITE);
    if (!mem)
        return nullptr;
...
bool Initialize()
{
    ...
    return g_NtAllocateVirtualMemory && g_NtProtectVirtualMemory && g_NtWriteVirtualMemory && g_NtCreateThreadEx && g_NtWaitForSingleObject && g_NtClose && g_NtFreeVirtualMemory;
}
